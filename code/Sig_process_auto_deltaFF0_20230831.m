...
%folderpath='F:\SynologyDrive\LAB-share\Member-HuJiaHao\LiLiang\Data\';
    folderpath='C:\Users\admin\Documents\WeChat Files\wxid_9gmw5koj7uso22\FileStorage\File\2023-09\';
    dataname='20230905Ctrl_NAc2site1_300uA single_2uM_deepcad_output';
    fs=10;%采样频率，单位Hz
    r_um=50;%用来框选的圆的半径,单位um
    pixsize=0.425;%像素尺寸大小，单位um/pix
    thresh=0.2;%colormap中最大值白色对应的ΔF/F0
    thresh_select=0.02;%荧光区域选取阈值
    bp1=1.5;%拟合的断点1
    bp2=5;%拟合的断点2
%%%%%%%%%%%%%%%%%%%%%%%以上为可调参数%%%%%%%%%%%%%%%%%%%%%%%
r_pix=r_um/pixsize;
...
maximg=max(delta_Image(:,:,5:14/dt),[],3); %看前5~14s内荧光变化量的最大值(即最大刺激程度)
...
colormap(mymap);
labeldis=linspace(0,1,int8(thresh/0.1+1));
label=0:0.1:thresh;
